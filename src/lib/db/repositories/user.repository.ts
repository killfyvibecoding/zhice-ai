...
import { authAccounts, users, resumes } from '../schema';
...
export const userRepository = {
  ...
  async findAuthAccount(provider: string, providerAccountId: string) {
    const result = await db.select().from(authAccounts).where(
      and(eq(authAccounts.provider, provider), eq(authAccounts.providerAccountId, providerAccountId)),
    ).limit(1);
    return result[0] || null;
  },

  async claimAuthAccount(provider: string, providerAccountId: string, userId: string) {
    const existing = await this.findAuthAccount(provider, providerAccountId);
    ...
    try {
      ...
      const concurrentClaim = await this.findAuthAccount(provider, providerAccountId);
  ...
  async linkWechat(id: string, openid: string, name?: string, avatarUrl?: string) {
    await db.update(users).set({
      fingerprint: `wechat:${openid}`,
      ...(name ? { name } : {}),
      ...(avatarUrl ? { avatarUrl } : {}),
      updatedAt: new Date(),
