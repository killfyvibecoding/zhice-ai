...
export class SQLiteAdapter implements DatabaseAdapter {
  ...
  private readonly isBuild = process.env.ZHICE_BUILD === 'true';
  ...
  constructor(path: string) {
    ...
    // Build workers only import server routes; database migrations belong to
    // runtime startup and must not run concurrently during page collection.
    if (!this.isBuild) {
      try {
