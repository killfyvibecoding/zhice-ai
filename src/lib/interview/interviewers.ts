...
const presets: Record<string, PresetInterviewer> = {
  hr: {
    zh: {
      ...
      systemPrompt: '',
      completionGuidance: '围绕动机、文化适配、职业规划和稳定性逐项核实；对含糊或矛盾之处继续追问，直到匹配度和主要风险判断清晰，再给出总结，不为凑问题数量继续提问。',
    },
    en: {
      type: 'hr',
      name: 'Lisa Wang',
      ...
      systemPrompt: '',
      completionGuidance: 'Verify motivation, cultural fit, career direction, and stability; keep probing vague or inconsistent answers until fit and major risks are clear, then summarize without extending the interview to fill a quota.',
    },
  },
  technical: {
    zh: {
      ...
      systemPrompt: '',
      completionGuidance: '沿基础原理、实现细节、边界条件和故障排查逐层深入；回答扎实可以跳级，回答含糊就追问到确认能力边界，关键技术证据充分后结束。',
    },
    en: {
      type: 'technical',
      name: 'Mike Zhang',
      ...
      systemPrompt: '',
      completionGuidance: 'Progress through fundamentals, implementation details, edge cases, and troubleshooting; skip ahead when answers are strong, probe until the capability boundary is clear when they are vague, and conclude once the key technical evidence is sufficient.',
    },
  },
  scenario: {
    zh: {
      ...
      systemPrompt: '',
      completionGuidance: '围绕一个岗位相关场景，按目标、约束、方案、权衡、容量、故障和复盘逐层追问；方案取舍与主要风险清晰后结束，若关键约束未覆盖则继续深入。',
    },
    en: {
      type: 'scenario',
      name: 'Kevin Wang',
      ...
      systemPrompt: '',
      completionGuidance: 'Use one role-relevant scenario and probe goals, constraints, design, trade-offs, capacity, failure handling, and retrospection; conclude when trade-offs and major risks are clear, continuing only when a key constraint remains unexplored.',
    },
  },
  behavioral: {
    zh: {
      ...
      systemPrompt: '',
      completionGuidance: '围绕岗位关键行为，以 STAR 核实背景、个人行动、决策和可验证结果；对泛化、包装或缺少个人贡献的回答继续追问，关键行为证据充分后总结。',
    },
    en: {
      type: 'behavioral',
      name: 'Fang Liu',
      ...
      systemPrompt: '',
      completionGuidance: 'Use STAR to verify context, personal actions, decisions, and measurable outcomes for role-critical behaviors; probe general, embellished, or unclear contributions until the evidence is sufficient, then summarize.',
    },
  },
  project_deep_dive: {
    zh: {
      ...
      systemPrompt: '',
      completionGuidance: '以简历项目为主线核实角色、决策、难点、结果和复盘；若个人贡献或技术判断不清就继续拆解，直到项目参与证据和结果可信，再结束本轮。',
    },
    en: {
      type: 'project_deep_dive',
      name: 'Gang Chen',
      ...
      systemPrompt: '',
      completionGuidance: 'Use the resume projects to verify role, decisions, challenges, outcomes, and retrospection; keep decomposing unclear ownership or technical judgment until the evidence is credible, then conclude.',
    },
  },
  leader: {
    zh: {
      ...
      systemPrompt: '',
      completionGuidance: '围绕业务影响、技术视野、团队领导、战略决策和职业规划动态取舍；对关键判断继续追问，对已经形成完整全局判断的回答直接总结，不要求固定问题数。',
    },
    en: {
      type: 'leader',
      name: 'David Zhao',
      ...
      systemPrompt: '',
      completionGuidance: 'Dynamically cover business impact, technical vision, leadership, strategic decisions, and career direction; probe important judgments, but conclude as soon as a coherent executive-level assessment is formed rather than targeting a fixed count.',
    },
  },
};
