...
import { extractDocxText, isDocxFile } from '@/lib/resume/word-parser';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isPdfFile(file) && !isDocx && !isImage) {
      return NextResponse.json(
        { error: 'Unsupported file type. Accepted: PDF or DOCX' },
        { status: 400 }
      );
    }
    ...
    if (isPdf) {
      ...
      if (!docxText) {
        return NextResponse.json({ error: 'Word 文档中没有可读取的文字内容' }, { status: 400 });
      }
      messages.push({
        role: 'user',
        content: `Below is the full text extracted from a Word resume. Extract all resume information using the EXACT JSON schema from the system prompt.\n\n---\n${docxText}\n---`,
      });
    } else {
