...
import { buildResumeTitle } from '@/lib/resume/title';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user) {
      return NextResponse.json({ error: '请先完成微信登录' }, { status: 401 });
    }
    ...
    if (request.headers.get('x-client') === 'miniprogram') {
      const account = await getMiniProgramAccount(user.id);
      if (account.credits < MINI_CREDIT_COSTS.generateResume) {
        return NextResponse.json({ error: MINI_INSUFFICIENT_CREDITS_MESSAGE }, { status: 402 });
      }
    }
    ...
    const templateContext = template ? `\nThe selected visual resume template is "${template}". Keep the content structured and concise so it renders well in this template.` : '';
    const promptText = `Generate a complete resume for a ${jobTitle} ${yearsOfExperience === 0 ? 'at entry level (fresh graduate / no prior experience)' : `with ${yearsOfExperience} years of experience`}.${skillsContext}${industryContext}${experienceContext}${templateContext}
...
Respond with JSON only.`;
    ...
    const personalInfo = (generatedData.personal_info || {}) as { fullName?: string };
    const resumeTitle = buildResumeTitle(personalInfo.fullName || '', jobTitle);
    ...
    if (request.headers.get('x-client') === 'miniprogram') {
      try {
        await consumeMiniProgramCredits(user.id, MINI_CREDIT_COSTS.generateResume);
      } catch (error) {
